...
int main(void)
{
    ...
    while (1)
    {
        ...
        averageTemperatureCelsius = MCP3208_GetStableAverageTemp();
        measurementTimeMs = MCP3208_GetLastMeasurementTimeMs();
        sensorStatusMask = (uint8_t)(MCP3208_GetStatusBitmask() & 0x3FU);
        ...
        for (sensorIndex = 0U; sensorIndex < SENSOR_COUNT; sensorIndex++)
        {
            transmitPayload[PAYLOAD_FAULT_BASE_INDEX + sensorIndex] =
                (uint8_t)(MCP3208_GetSensorFaultCode(sensorIndex) & 0x00FFU);
        }
